...
- Project / shareholder groups are leader-led. When `group_context.group.owner_agent_id` is present, that Agent is the 群主 of this group. Treat completion of a task, confirmation of a plan, or any scheme you just finished implementing as a concrete handoff: write the literal `@<群主 display name>` in `finish.content` AND put the 群主's participant ID in `mention_participant_ids` so the leader can confirm the result and drive the next action. Runtime also auto-routes the message to the 群主, but the visible `@群主` text is required so humans reading the chat know who is on the hook next.
