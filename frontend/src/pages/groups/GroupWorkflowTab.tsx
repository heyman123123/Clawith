            ...
            <div className="workflow-gate">
                ...
                <div>
                    ...
                    {pendingDecisions.map((decision) => (
                        <div key={String(decision.id)} style={{ marginTop: 8 }}>
                            <span>{String(decision.title)} · {String(decision.category)}</span>
                            <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                                <button type="button" onClick={() => approveDecision.mutate(String(decision.id))}>批准</button>
                                <button type="button" className="quiet" onClick={() => rejectDecision.mutate(String(decision.id))}>拒绝</button>
                            </div>
                        </div>
                    ))}
                </div>
